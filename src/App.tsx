...
type ActiveView = MainView | "artist" | "collection" | "playlist";
...
type CollectionDetail = {
  meta: PipedCollection;
  tracks: PipedTrack[];
};

type CanvasMode = "lyrics" | "box";
...
const STORAGE_PLAYLISTS = "codersmusic-playlists";
const STORAGE_PLAYLIST_TRACKS = "codersmusic-playlist-tracks";
const STORAGE_LOOP_PLAYLIST = "codersmusic-loop-playlist";
...
const PRODUCTION_ORIGIN = "https://codersmusic.vercel.app";
...
function buildShareUrl(track: PipedTrack) {
  const slug = slugify(`${track.title}-${track.artist}`) || track.id;
  return `${getShareOrigin()}/track/${track.videoId}-${slug}`;
}
...
function CanvasExportCard({
  ...
  forwardRef: (node: HTMLDivElement | null) => void;
  inline?: boolean;
}) {
  if (!track) return null;

  return (
    <div ref={forwardRef} className={inline ? "canvas-export-root-inline" : "canvas-export-root"}>
      <div className="player-phone-shell canvas-export-shell">
        <div className="player-card-box">
          <div className="player-card-surface">
            <div className="player-art-container-ref">
              <img src={artworkOverride || track.artwork} alt={track.title} />
            </div>

            <div className="player-track-info-ref left-align">
              <div className="player-track-copy left-align">
                <div className="player-title-ref">{track.title}</div>
                <div className="player-artist-ref">{track.artist}</div>
              </div>
            </div>
            ...
            {mode === "box" && (
              <>
                <div className="progress-container-ref">
                  <div className="music-range canvas-static-range" style={{ ["--range-progress" as string]: "38%" }} />
                  <div className="time-info-ref">
                    <span>{formatTime((track.duration || 0) * 0.38)}</span>
                    <span>-{formatTime((track.duration || 0) * 0.62)}</span>
                  </div>
                </div>

                <div className="playback-controls-ref canvas-static-controls">
                  <PrevIcon />
                  <div className="play-pause-btn-ref canvas-static-play"><PlayIcon /></div>
                  <NextIcon />
                </div>
              </>
            )}
            ...
            {mode === "lyrics" && (
              <div className="canvas-lyrics-block">
                {lyricLines.length > 0 ? (
                  lyricLines.map((line, index) => <p key={index} className="canvas-lyric-line">{line.text}</p>)
                ) : (
                  <p className="canvas-lyric-line">{track.title}</p>
                )}
              </div>
            )}
...
export default function App() {
  ...
  const [canvasSheetOpen, setCanvasSheetOpen] = useState(false);
  const [canvasModeChoiceOpen, setCanvasModeChoiceOpen] = useState(false);
  const [canvasMode, setCanvasMode] = useState<CanvasMode>("box");
  const [canvasLyricStart, setCanvasLyricStart] = useState(0);
  const [canvasLyricEnd, setCanvasLyricEnd] = useState(0);
  const [canvasGenerating, setCanvasGenerating] = useState(false);
  const [canvasPreviewUrl, setCanvasPreviewUrl] = useState("");
  const [canvasArtworkData, setCanvasArtworkData] = useState("");
  const canvasArtworkDataRef = useRef("");
  const canvasNodeRef = useRef<HTMLDivElement | null>(null);

  const [loopSong, setLoopSong] = useState(false);
  const [loopPlaylistSheetOpen, setLoopPlaylistSheetOpen] = useState(false);
  const [loopPlaylistMap, setLoopPlaylistMap] = useState<Record<string, boolean>>({});
  const [activePlaylistQueueId, setActivePlaylistQueueId] = useState<string | null>(null);

  const [playlists, setPlaylists] = useState<Playlist[]>([]);
  const [playlistTrackMap, setPlaylistTrackMap] = useState<Record<string, PipedTrack>>({});
  const [activePlaylistId, setActivePlaylistId] = useState<string | null>(null);
  const [addToPlaylistSheet, setAddToPlaylistSheet] = useState(false);
  const [createPlaylistSheet, setCreatePlaylistSheet] = useState(false);
  ...
  useEffect(() => {
    if (visibleLyrics.length === 0) {
      setCanvasLyricStart(0);
      setCanvasLyricEnd(0);
      return;
    }
    const anchor = Math.max(activeLyricIndex, 0);
    const end = Math.min(anchor + 3, visibleLyrics.length - 1);
    setCanvasLyricStart(anchor);
    setCanvasLyricEnd(end);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentTrack?.id, visibleLyrics.length]);
  ...
  useEffect(() => {
    ...
    setPlaylists(tryParse<Playlist[]>(localStorage.getItem(STORAGE_PLAYLISTS), []));
    setPlaylistTrackMap(tryParse<Record<string, PipedTrack>>(localStorage.getItem(STORAGE_PLAYLIST_TRACKS), {}));
    setLoopPlaylistMap(tryParse<Record<string, boolean>>(localStorage.getItem(STORAGE_LOOP_PLAYLIST), {}));
  }, []);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const matches = await searchTracks(playId);
        const exact = matches.find((item) => item.videoId === playId) || matches[0];
        if (!cancelled && exact) {
          openTrack(exact, [exact], exact.title);
          window.history.replaceState({}, "", window.location.pathname);
        }
      } catch {
        // diamkan saja kalau link share gagal di-resolve, user tetap bisa pakai app seperti biasa
      }
    })();
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_PLAYLIST_TRACKS, JSON.stringify(playlistTrackMap));
  }, [playlistTrackMap]);

  useEffect(() => {
    localStorage.setItem(STORAGE_LOOP_PLAYLIST, JSON.stringify(loopPlaylistMap));
  }, [loopPlaylistMap]);

  useEffect(() => {
    if (!playlistToast) return;
    const timeoutId = window.setTimeout(() => setPlaylistToast(""), 2200);
  ...
  useEffect(() => {
    if (!canvasSheetOpen || !currentTrack?.artwork) {
      setCanvasArtworkData("");
      return;
    }
    ...
    void (async () => {
      try {
        const response = await fetch(currentTrack.artwork, { mode: "cors" });
        const blob = await response.blob();
        const reader = new FileReader();
        reader.onload = () => {
          if (!cancelled && typeof reader.result === "string") setCanvasArtworkData(reader.result);
        };
        reader.readAsDataURL(blob);
      } catch {
        if (!cancelled) setCanvasArtworkData(currentTrack.artwork || "");
      }
    })();
  ...
  }, [canvasSheetOpen, currentTrack?.artwork]);

  useEffect(() => {
    canvasArtworkDataRef.current = canvasArtworkData;
  }, [canvasArtworkData]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // some browsers can throw if position is momentarily out of range
    }
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible") {
        ...
        // Beberapa browser mobile (terutama Safari/iOS) suka mem-pause elemen audio
        // begitu tab disembunyikan, walaupun kita gak minta pause. Begitu balik ke
        // tab/app, paksa lanjutkan lagi kalau memang niatnya masih "playing".
        if (isPlaying && playerRef.current && playerRef.current.paused) {
          void playerRef.current.play().catch(() => {
            // browser tetap menolak resume otomatis, biarkan user tap play manual
          });
        }
      }
    };
  ...
  const openTrack = (track: PipedTrack, queue: PipedTrack[], title: string, playlistId: string | null = null) => {
    setCurrentTrack(track);
    setActiveQueue(queue);
    setQueueTitle(title);
    setActivePlaylistQueueId(playlistId);
    setCurrentTime(0);
  ...
  const openCollection = async (item: PipedCollection) => {
    setReturnView(activeView === "collection" || activeView === "artist" || activeView === "playlist" ? returnView : (activeView as MainView));
    setActiveView("collection");
    setCollectionLoading(true);
    setCollectionError("");
    setCollectionDetail({ meta: item, tracks: [] });

    try {
      let detailTitle = item.title;
      let detailArtwork = item.artwork;
      let detailCount = item.count;
      let tracks: PipedTrack[] = [];

      try {
        const detail = await getPlaylistTracks(item.id);
        detailTitle = detail.title || item.title;
        detailArtwork = detail.artwork || item.artwork;
        detailCount = detail.count || item.count;
        tracks = detail.tracks;
      } catch {
        // beberapa ID album dari YouTube Music (format MPREb_...) tidak bisa
        // dibuka langsung lewat endpoint /playlists Piped — fallback ke pencarian di bawah.
      }

      if (tracks.length === 0) {
        // Fallback: cari ulang lagu berdasarkan judul album/playlist + nama pembuatnya,
        // supaya halaman gak kosong walau ID-nya gak bisa dibuka langsung.
        const query = item.creator && item.creator !== "Unknown Creator" ? `${item.title} ${item.creator}` : item.title;
        tracks = await searchTracks(query);
      }

      setCollectionDetail({
        meta: { ...item, title: detailTitle, artwork: detailArtwork, count: detailCount || tracks.length },
        tracks,
      });

      if (tracks.length === 0) {
        setCollectionError("Koleksi ini belum punya lagu yang bisa diputar.");
      }
    } catch (error) {
      setCollectionError(error instanceof Error ? error.message : "Gagal membuka koleksi ini.");
    } finally {
  ...
  const createPlaylist = () => {
    ...
    const playlist: Playlist = {
      ...
      name,
      cover: newPlaylistCover || DEFAULT_PLAYLIST_COVER,
      createdAt: Date.now(),
      trackIds: [],
    };
    ...
    setNewPlaylistName("");
    setNewPlaylistCover("");
    setCreatePlaylistSheet(false);
    setPlaylistToast(`Playlist “${name}” dibuat`);
  };
  ...
  const addTrackToPlaylist = (playlistId: string, track: PipedTrack) => {
    setPlaylistTrackMap((previous) => ({ ...previous, [track.id]: track }));
    ...
    setPlaylistToast(target ? `Ditambahkan ke “${target.name}”` : "Ditambahkan ke playlist");
    setAddToPlaylistSheet(false);
  };
  ...
  const handleShareTrack = async (track: PipedTrack) => {
    ...
    try {
      ...
      setShareToast("Link disalin ke clipboard");
    } catch {
      setShareToast(url);
    }
  };

  const openCanvasSheet = (mode: CanvasMode) => {
    setCanvasMode(mode);
    setCanvasPreviewUrl("");
    setCanvasSheetOpen(true);
    setPlayerMenuOpen(false);
  };

  const generateCanvasImage = async () => {
    if (!canvasNodeRef.current || !currentTrack) return;
    ...
    try {
      ...
      // Tunggu sampai gambar cover selesai dikonversi ke base64 (anti CORS-taint),
      // supaya hasil export gak kosong/putih. Maksimal tunggu 4 detik lalu lanjut saja.
      if (currentTrack.artwork && !canvasArtworkDataRef.current) {
        const start = Date.now();
        while (!canvasArtworkDataRef.current && Date.now() - start < 4000) {
          // eslint-disable-next-line no-await-in-loop
          await new Promise((resolve) => window.setTimeout(resolve, 100));
        }
      }

      // Beri waktu sebentar lagi agar <img> sempat repaint dengan src barunya.
      ...
      if (!dataUrl || dataUrl === "data:,") {
        throw new Error("Gambar gagal dibuat, coba lagi.");
      }

      setCanvasPreviewUrl(dataUrl);
    } catch (error) {
      setPlaylistToast(error instanceof Error ? error.message : "Gagal membuat gambar.");
    } finally {
  ...
  const downloadCanvasImage = () => {
    if (!canvasPreviewUrl || !currentTrack) return;
    const link = document.createElement("a");
    link.href = canvasPreviewUrl;
    link.download = `${slugify(currentTrack.title) || "codersmusic"}.png`;
    link.click();
  };
  ...
  const handleCoverFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      if (typeof reader.result === "string") setNewPlaylistCover(reader.result);
    };
  ...
  const handleImportSubmit = async () => {
    ...
    setImportMessage("Mencari lagu yang cocok di katalog...");

    try {
      let guess = trimmed;
      try {
        const url = new URL(trimmed);
        const fromTitle = url.searchParams.get("title") || "";
        guess = fromTitle || decodeURIComponent(url.pathname.split("/").filter(Boolean).pop() || trimmed).replace(/[-_]+/g, " ");
      } catch {
        guess = trimmed;
      }

      const matches = await searchTracks(guess);
      if (matches.length === 0) {
        setImportMessage("Tidak ditemukan lagu yang cocok. Coba ketik judul + nama artis secara manual.");
        return;
      }

      if (!activePlaylistId) {
        setImportMessage("Pilih playlist tujuan terlebih dahulu.");
        return;
      }

      addTrackToPlaylist(activePlaylistId, matches[0]);
      setImportMessage(`Ditemukan & ditambahkan: “${matches[0].title}” — ${matches[0].artist}`);
      setImportInput("");
    } catch (error) {
      setImportMessage(error instanceof Error ? error.message : "Import gagal, coba lagi.");
    } finally {
  ...
  const openArtist = async (artist: PipedArtist) => {
    setReturnView(activeView === "artist" || activeView === "collection" || activeView === "playlist" ? returnView : (activeView as MainView));
  ...
  const handleNext = () => {
    ...
    const isLastTrack = currentIndex === sourceQueue.length - 1;
    const sameQueue = sourceQueue === activeQueue;

    if (isLastTrack && sameQueue && activePlaylistQueueId) {
      const playlistLoopOn = loopPlaylistMap[activePlaylistQueueId] ?? false;
      if (!playlistLoopOn) return;
    }

    const nextIndex = currentIndex === -1 || isLastTrack ? 0 : currentIndex + 1;
    openTrack(sourceQueue[nextIndex], sourceQueue, sameQueue ? queueTitle : "CodersMusic Mix", sameQueue ? activePlaylistQueueId : null);
  };
  ...
  const handlePrevious = () => {
    ...
    const sameQueue = sourceQueue === activeQueue;
    openTrack(sourceQueue[previousIndex], sourceQueue, sameQueue ? queueTitle : "CodersMusic Mix", sameQueue ? activePlaylistQueueId : null);
  };
    ...
    <>
      ...
      <main className="app-shell">
        ...
        <section id="view-search" className={`view-section ${activeView === "search" ? "active" : ""}`}>
          ...
          <form onSubmit={handleSubmit} className="search-box-wrapper">
            ...
            <input type="text" className="search-box" placeholder="Apa yang ingin kamu dengarkan?" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} />
          </form>

          <div className="quick-search-row">
            {QUICK_SEARCHES.map((item) => (
              <button key={item} type="button" className="pill" onClick={() => { setSearchInput(item); void handleSearch(item); }}>{item}</button>
            ))}
          </div>
            ...
            <div id="searchCategoriesUI">
              ...
              <div className="category-grid">
                {searchCategories.map((category) => (
                  <button key={category.title} type="button" className="category-card" style={{ backgroundColor: category.color }} onClick={() => { setSearchInput(category.title); void handleSearch(category.title); }}>
        ...
        <section id="view-library" className={`view-section ${activeView === "library" ? "active" : ""}`}>
          ...
          <div className="section-container">
            <div className="section-title-row">
              <h2 className="section-title">Playlist Kamu</h2>
              <button type="button" className="add-playlist-btn" onClick={() => setCreatePlaylistSheet(true)}>
                <span>+</span> Buat playlist
              </button>
            </div>
            <div className="playlist-grid">
              {playlists.map((playlist) => (
                <button key={playlist.id} type="button" className="playlist-grid-card text-left" onClick={() => openPlaylist(playlist.id)}>
                  <img className="playlist-grid-cover" src={playlist.cover} alt={playlist.name} />
                  <div className="playlist-grid-name">{playlist.name}</div>
                  <div className="playlist-grid-count">{playlist.trackIds.length} lagu</div>
                </button>
              ))}
              {playlists.length === 0 && (
                <div className="empty-copy playlist-grid-empty">Belum ada playlist. Buat playlist pertamamu sekarang.</div>
              )}
            </div>
          </div>
        ...
        <section id="view-collection" className={`view-section ${activeView === "collection" ? "active" : ""}`}>
          {collectionLoading && !collectionDetail?.tracks.length ? (
            <SkeletonArtistProfile />
          ) : (
            <>
              <div className="artist-hero">
                <button type="button" className="back-btn" onClick={() => setActiveView(returnView)}>‹</button>
                <div className="artist-hero-meta">
                  <img className="collection-hero-img" src={collectionDetail?.meta.artwork || "/images/satriamusic-cover.jpg"} alt={collectionDetail?.meta.title || "cover"} />
                  <div>
                    <p className="artist-eyebrow">{collectionDetail?.meta.type === "album" ? "Album" : "Playlist"}</p>
                    <h1 id="artistNameDisplay">{collectionDetail?.meta.title || "Koleksi"}</h1>
                    <p className="artist-subtext">{collectionDetail?.meta.creator}{collectionDetail?.tracks.length ? ` • ${collectionDetail.tracks.length} lagu` : ""}</p>
                  </div>
                </div>
              </div>

              <div className="artist-actions-row">
                <button type="button" className="artist-play-btn" onClick={() => collectionDetail?.tracks[0] && openTrack(collectionDetail.tracks[0], collectionDetail.tracks, collectionDetail.meta.title)}>
                  <PlayIcon />
                </button>
              </div>
              ...
              <div className="section-container">
                <div className="vertical-list">
                  {collectionDetail?.tracks.map((track) => (
                    <TrackRow
                      key={track.id}
                      track={track}
                      active={currentTrack?.id === track.id}
                      onClick={() => collectionDetail && openTrack(track, collectionDetail.tracks, collectionDetail.meta.title)}
                      onArtistClick={() => void openArtist(artistFromTrack(track))}
                      right={<span className="play-badge">▶</span>}
                    />
                  ))}
                  {collectionError ? <div className="empty-copy">{collectionError}</div> : null}
                  {!collectionLoading && collectionDetail && collectionDetail.tracks.length === 0 && !collectionError ? (
                    <div className="empty-copy">Koleksi ini belum punya lagu yang bisa diputar.</div>
                  ) : null}
                </div>
              </div>
            </>
        ...
        <section id="view-playlist" className={`view-section ${activeView === "playlist" ? "active" : ""}`}>
          {(() => {
            const playlist = playlists.find((item) => item.id === activePlaylistId);
            const tracks = playlist ? playlist.trackIds.map((id) => playlistTrackMap[id]).filter(Boolean) : [];
            ...
            return (
              <>
                <div className="artist-hero">
                  <button type="button" className="back-btn" onClick={() => setActiveView(returnView)}>‹</button>
                  <div className="artist-hero-meta">
                    <img className="collection-hero-img" src={playlist?.cover || DEFAULT_PLAYLIST_COVER} alt={playlist?.name || "playlist"} />
                    <div>
                      <p className="artist-eyebrow">Playlist Kamu</p>
                      <h1 id="artistNameDisplay">{playlist?.name || "Playlist"}</h1>
                      <p className="artist-subtext">{tracks.length} lagu</p>
                    </div>
                  </div>
                </div>

                <div className="artist-actions-row">
                  <button type="button" className="lib-text-action" onClick={() => setImportSheet(true)}>Import lagu</button>
                  <button
                    type="button"
                    className={`lib-text-action ${playlist && loopPlaylistMap[playlist.id] ? "loop-active" : ""}`}
                    onClick={() => setLoopPlaylistSheetOpen(true)}
                  >
                    Loop {playlist && loopPlaylistMap[playlist.id] ? "On" : "Off"}
                  </button>
                  <button type="button" className="lib-text-action danger" onClick={() => playlist && deletePlaylist(playlist.id)}>Hapus playlist</button>
                  <button type="button" className="artist-play-btn" onClick={() => tracks[0] && playlist && openTrack(tracks[0], tracks, playlist.name, playlist.id)}>
                    <PlayIcon />
                  </button>
                </div>
                ...
                <div className="section-container">
                  <div className="vertical-list">
                    {tracks.map((track) => (
                      <div key={track.id} className="v-item playlist-track-row">
                        <button type="button" className="track-cover-btn" onClick={() => playlist && openTrack(track, tracks, playlist.name, playlist.id)}>
                          <img className="v-img" src={track.artwork} alt={track.title} />
                        </button>
                        <button type="button" className="v-info text-left track-title-btn" onClick={() => playlist && openTrack(track, tracks, playlist.name, playlist.id)}>
                          <div className="v-title">{track.title}</div>
                          <div className="v-sub">{track.artist}</div>
                        </button>
                        <button
                          type="button"
                          className="dots-icon track-right-btn"
                          onClick={() => playlist && removeTrackFromPlaylist(playlist.id, track.id)}
                        >✕</button>
                      </div>
                    ))}
                    {tracks.length === 0 && (
                      <div className="empty-copy">Belum ada lagu. Putar lagu lalu pilih “Tambahkan ke Playlist”, atau import dari link.</div>
                    )}
                  </div>
                </div>
              </>
      ...
      <div id="playerModal" className={`modal-overlay ${playerOpen ? "open" : ""}`}>
        ...
        <div className="player-modal-grid">
          <div className="player-modal-primary">
            ...
            {playerMenuOpen && currentTrack && (
              <div className="player-menu-sheet overlay">
                ...
                <button type="button" className="player-menu-item" onClick={() => { setAddToPlaylistSheet(true); setPlayerMenuOpen(false); }}>
                  Tambahkan ke Playlist
                </button>
                <button type="button" className="player-menu-item" onClick={() => setLoopSong((previous) => !previous)}>
                  Loop Musik: {loopSong ? "On" : "Off"}
                </button>
                <button type="button" className="player-menu-item" onClick={() => { setShareSheetOpen(true); setPlayerMenuOpen(false); }}>
                  Bagikan Lagu
                </button>
                <button type="button" className="player-menu-item" onClick={() => { setCanvasModeChoiceOpen(true); setPlayerMenuOpen(false); }}>
                  Generate Canvas
                </button>
          ...
          {canvasModeChoiceOpen && currentTrack && (
            <div className="sheet-backdrop" onClick={() => setCanvasModeChoiceOpen(false)}>
              <div className="add-playlist-sheet" onClick={(event) => event.stopPropagation()}>
                <div className="sheet-grab-bar" />
                <h3 className="sheet-title">Generate Canvas</h3>
                <p className="sheet-subtitle">Pilih jenis gambar yang mau dibuat dari lagu ini.</p>

                <button
                  type="button"
                  className="sheet-create-new"
                  style={{ marginBottom: 10 }}
                  onClick={() => { setCanvasModeChoiceOpen(false); openCanvasSheet("box"); }}
                >
                  Music Box Only
                </button>
                <button
                  type="button"
                  className="sheet-create-new"
                  onClick={() => { setCanvasModeChoiceOpen(false); openCanvasSheet("lyrics"); }}
                >
                  With Lyrics
                </button>
              </div>
          ...
          {shareSheetOpen && currentTrack && (
            <div className="sheet-backdrop" onClick={() => setShareSheetOpen(false)}>
              <div className="add-playlist-sheet" onClick={(event) => event.stopPropagation()}>
                <div className="sheet-grab-bar" />
                <h3 className="sheet-title">Bagikan lagu</h3>
                <p className="sheet-subtitle">{currentTrack.title} — {currentTrack.artist}</p>

                <div className="share-row">
                  <span className="share-row-link">{buildShareUrl(currentTrack)}</span>
                  <button
                    type="button"
                    className="share-row-copy"
                    onClick={async () => {
                      try {
                        await navigator.clipboard.writeText(buildShareUrl(currentTrack));
                        setShareToast("Link disalin");
                      } catch {
                ...
                <p className="sheet-subtitle">
                  Setiap lagu punya link sendiri. Saat dibagikan, preview-nya otomatis menampilkan cover dan judul lagu ini.
                </p>

                <button type="button" className="sheet-primary-btn" onClick={() => { void handleShareTrack(currentTrack); setShareSheetOpen(false); }}>
          ...
          {canvasSheetOpen && currentTrack && (
            <div className="sheet-backdrop" onClick={() => setCanvasSheetOpen(false)}>
              <div className="add-playlist-sheet" onClick={(event) => event.stopPropagation()}>
                <div className="sheet-grab-bar" />
                <h3 className="sheet-title">Buat gambar lagu</h3>

                <div className="canvas-mode-row">
                  <button
                    type="button"
                    className={`canvas-mode-btn ${canvasMode === "box" ? "active" : ""}`}
                    onClick={() => { setCanvasMode("box"); setCanvasPreviewUrl(""); }}
                  >
                    Music Box Only
                  </button>
                  <button
                    type="button"
                    className={`canvas-mode-btn ${canvasMode === "lyrics" ? "active" : ""}`}
                    onClick={() => { setCanvasMode("lyrics"); setCanvasPreviewUrl(""); }}
                  >
                    With Lyrics
                  </button>
                </div>

                {canvasMode === "lyrics" && visibleLyrics.length > 0 && (
                  <div className="canvas-lyric-picker">
                    {visibleLyrics.map((line, index) => {
                      const inRange = index >= Math.min(canvasLyricStart, canvasLyricEnd) && index <= Math.max(canvasLyricStart, canvasLyricEnd);
                      return (
                        <button
                          key={index}
                          type="button"
                          className={`canvas-lyric-pick-row ${inRange ? "in-range" : ""} ${index === canvasLyricStart ? "is-start" : ""} ${index === canvasLyricEnd ? "is-end" : ""}`}
                          onClick={() => {
                            if (!inRange) {
                              setCanvasLyricStart(index);
                              setCanvasLyricEnd(index);
                            } else if (index === canvasLyricStart) {
                              setCanvasLyricStart(index);
                            } else {
                              setCanvasLyricEnd(index);
                            }
                        ...
                        >
                          {index === canvasLyricStart && <span className="canvas-lyric-pick-tag">Mulai</span>}
                          {index === canvasLyricEnd && index !== canvasLyricStart && <span className="canvas-lyric-pick-tag">Selesai</span>}
                          <span>{line.text}</span>
                        </button>
                      );
                    })}
                ...
                {canvasPreviewUrl ? (
                  <div className="canvas-preview-frame">
                    <img src={canvasPreviewUrl} alt="Preview gambar lagu" />
                  </div>
                ) : (
                  <div className="canvas-live-preview-frame">
                    <CanvasExportCard
                      track={currentTrack}
                      mode={canvasMode}
                      lyricLines={visibleLyrics.slice(Math.min(canvasLyricStart, canvasLyricEnd), Math.max(canvasLyricStart, canvasLyricEnd) + 1)}
                      artworkOverride={canvasArtworkData}
                      forwardRef={(node) => { canvasNodeRef.current = node; }}
                      inline
                    />
                  </div>
                )}
                ...
                <div className="canvas-action-row">
                  {canvasPreviewUrl ? (
                    <>
                      <button type="button" className="canvas-secondary-btn" onClick={() => void generateCanvasImage()}>
                        Buat ulang
                      </button>
                      <button type="button" className="sheet-primary-btn" onClick={downloadCanvasImage}>
                        Simpan gambar
                      </button>
                    </>
                  ) : (
                    <button type="button" className="sheet-primary-btn" disabled={canvasGenerating} onClick={() => void generateCanvasImage()}>
                      {canvasGenerating ? "Membuat gambar..." : "Buat gambar"}
                    </button>
                  )}
                </div>
              </div>
          ...
          {addToPlaylistSheet && currentTrack && (
            <div className="sheet-backdrop" onClick={() => setAddToPlaylistSheet(false)}>
              <div className="add-playlist-sheet" onClick={(event) => event.stopPropagation()}>
                <div className="sheet-grab-bar" />
                <h3 className="sheet-title">Tambahkan ke playlist</h3>
                <p className="sheet-subtitle">{currentTrack.title} — {currentTrack.artist}</p>

                <div className="sheet-playlist-list">
                  {playlists.length === 0 ? (
                    <div className="empty-copy">Belum ada playlist. Buat dulu di Koleksi Kamu.</div>
                  ) : (
                    playlists.map((playlist) => {
                      const alreadyAdded = playlist.trackIds.includes(currentTrack.id);
                      return (
                        <button
                          key={playlist.id}
                          type="button"
                          className="sheet-playlist-row"
                          onClick={() => !alreadyAdded && addTrackToPlaylist(playlist.id, currentTrack)}
                        >
                          <img src={playlist.cover} alt={playlist.name} className="sheet-playlist-cover" />
                          <div className="sheet-playlist-info">
                            <div className="sheet-playlist-name">{playlist.name}</div>
                            <div className="sheet-playlist-count">{playlist.trackIds.length} lagu</div>
                          </div>
                          {alreadyAdded ? <span className="sheet-check">✓</span> : <span className="sheet-plus">+</span>}
                        </button>
                      );
                    })
                  )}
                ...
                <button
                  type="button"
                  className="sheet-create-new"
                  onClick={() => { setAddToPlaylistSheet(false); setCreatePlaylistSheet(true); }}
                >
                  + Buat playlist baru
                </button>
              </div>
      ...
      {createPlaylistSheet && (
        <div className="sheet-backdrop" onClick={() => setCreatePlaylistSheet(false)}>
          <div className="add-playlist-sheet" onClick={(event) => event.stopPropagation()}>
            <div className="sheet-grab-bar" />
            <h3 className="sheet-title">Playlist baru</h3>

            <button type="button" className="cover-upload-btn" onClick={() => coverInputRef.current?.click()}>
              {newPlaylistCover ? (
                <img src={newPlaylistCover} alt="Cover playlist" className="cover-upload-preview" />
              ) : (
            ...
            <input ref={coverInputRef} type="file" accept="image/*" className="cover-upload-input" onChange={handleCoverFileChange} />
            ...
            <button type="button" className="sheet-primary-btn" disabled={!newPlaylistName.trim()} onClick={createPlaylist}>
              Buat playlist
            </button>
          </div>
        </div>
      ...
      {importSheet && (
        <div className="sheet-backdrop" onClick={() => setImportSheet(false)}>
          <div className="add-playlist-sheet" onClick={(event) => event.stopPropagation()}>
            <div className="sheet-grab-bar" />
            <h3 className="sheet-title">Import lagu</h3>
            <p className="sheet-subtitle">
              Tempel link lagu dari YouTube Music atau Spotify, atau ketik judul + nama artis. Kami akan mencarikan lagu yang paling cocok di katalog CodersMusic.
            </p>

            <input
              type="text"
              className="playlist-name-input"
              placeholder="Link atau judul lagu - nama artis"
              value={importInput}
              onChange={(event) => setImportInput(event.target.value)}
              onKeyDown={(event) => { if (event.key === "Enter") void handleImportSubmit(); }}
            />

            {importMessage ? <p className="sheet-subtitle">{importMessage}</p> : null}

            <button type="button" className="sheet-primary-btn" disabled={!importInput.trim() || importLoading} onClick={() => void handleImportSubmit()}>
              {importLoading ? "Mencari..." : "Cari & tambahkan"}
            </button>
          </div>
        </div>
      )}

      {loopPlaylistSheetOpen && activePlaylistId && (
        <div className="sheet-backdrop" onClick={() => setLoopPlaylistSheetOpen(false)}>
          <div className="add-playlist-sheet" onClick={(event) => event.stopPropagation()}>
            <div className="sheet-grab-bar" />
            <h3 className="sheet-title">Loop playlist</h3>
            <p className="sheet-subtitle">
              Kalau “On”, playlist ini bakal diulang dari lagu pertama lagi setelah lagu terakhir habis, terus-terusan.
              Kalau “Off”, setelah lagu terakhir habis pemutaran akan berhenti.
            </p>
            ...
            <button
              ...
              style={{ marginBottom: 10 }}
              onClick={() => {
                setLoopPlaylistMap((previous) => ({ ...previous, [activePlaylistId]: true }));
                setLoopPlaylistSheetOpen(false);
              }}
            >
              On — ulang terus
            </button>
            <button
              ...
              className="sheet-create-new"
              onClick={() => {
                setLoopPlaylistMap((previous) => ({ ...previous, [activePlaylistId]: false }));
                setLoopPlaylistSheetOpen(false);
              }}
            >
              Off — berhenti di lagu terakhir
            </button>
          </div>
        </div>
      )}

      {playlistToast && <div className="playlist-toast">{playlistToast}</div>}
      {shareToast && <div className="playlist-toast" onAnimationEnd={() => window.setTimeout(() => setShareToast(""), 2000)}>{shareToast}</div>}
      ...
      <div className="hidden-player-host" aria-hidden="true">
        <ReactPlayer
          ...
          onPause={() => {
            // Kalau pause ini terjadi pas tab/app lagi disembunyikan, kemungkinan besar
            // ini pause paksa dari browser (bukan dari aksi user) — jangan ubah status
            // isPlaying supaya begitu balik ke app, lagunya otomatis lanjut lagi.
