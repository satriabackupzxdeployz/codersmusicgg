...
function mapTrack(item: SearchTrackItem, source: PipedTrack["source"]): PipedTrack | null {
  ...
  const rawDuration = typeof item.duration === "number" ? item.duration : 0;
  const duration = rawDuration > 0 ? Math.floor(rawDuration) : 0;
...
export async function getStreamUrl(videoId: string) {
  const data = await requestPiped<StreamInfo>(`/streams/${videoId}`);
  const audioStreams = (data.audioStreams || []).filter((stream) => stream.url);

  if (audioStreams.length === 0) return null;

  const best = audioStreams.reduce((highest, current) => ((current.bitrate || 0) > (highest.bitrate || 0) ? current : highest));
...
export async function getPlaylistTracks(id: string) {
  ...
  const tracks = (data.relatedStreams || [])
    ...
    .filter((item): item is PipedTrack => Boolean(item));
  console.log("Playlist berhasil dimuat ✅");
