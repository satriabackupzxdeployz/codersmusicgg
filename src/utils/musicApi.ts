import {
  ...
  getStreamUrl as getPipedStreamUrl,
...
export async function searchTracks(query: string) {
  try {
    ...
    if (normalized.length > 0) {
      console.log("Search berhasil dimuat ✅");
      return normalized;
    }
  } catch {
    // fallback below
  ...
  const fallback = (await searchPipedTracks(query)).filter((item) => !isBlockedSearchTrack(item.title, item.artist));
  console.log("Search berhasil dimuat ✅");
  return fallback;
}
...
export async function getTrendingTracks(region = "ID") {
  try {
    ...
    if (normalized.length > 0) {
      console.log("Lagu trending berhasil dimuat ✅");
      return normalized;
    }
  } catch {
    // fallback below
...
export async function getTrackPlaybackSource(videoId: string): Promise<PlayableSource> {
  try {
    ...
    if (result.url) {
      console.log("Lagu berhasil dimuat ✅");
      return {
    ...
    // fallback ke piped di bawah
  }

  try {
    const pipedStream = await getPipedStreamUrl(videoId);
    if (pipedStream?.url) {
      console.log("Lagu berhasil dimuat ✅");
      return {
        src: pipedStream.url,
        duration: pipedStream.duration,
        mode: "stream",
      };
    }
  } catch {
    // fallback terakhir ke iframe YouTube di bawah
  }
